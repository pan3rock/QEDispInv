...
**MPI Execution**: For faster execution with multiple initial models, use MPI:
```bash
cd demo/lvl-l4
...
**MPI Execution**: For faster execution with multiple initial models, use MPI:
```bash
cd demo/syn-crustmantle
mpirun -n 8 ../../bin/inversion -d data.txt
```

*Note*: This example uses 100 initial models, so using more MPI processes (e.g., 8-16) will provide better parallelization.

# Tutorial

**Note on Parallel Execution**: The inversion module supports MPI-based parallelization for distributed execution across multiple CPU cores. See the [Inversion Tutorial](./inversion.md) for detailed instructions on running with `mpirun`.
